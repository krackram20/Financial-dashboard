...
ui <-   dashboardPage(
    ...
    dashboardBody( tabItems( tabItem(tabName = "Stock",  fluidRow(
       
    
        
        
        # Show a plot of the generated distribution
        box(width = 10,title = "Historico de Precios", status = "success", solidHeader = TRUE,
            collapsible = TRUE, dropdownButton( icon = icon("sliders"),
                                               radioButtons("tipo_grafica", 'Tipo de Grafica', choices = c('Line', 'Candle-stick'))),
            selectInput("download", "Select a stock (SPY 500 only)", choices = Tix), plotOutput("distPlot")), 
        
        box(width = 10,title = "Precios",  status = 'success',
            collapsible = TRUE, dataTableOutput('table')))), 
        
        tabItem(tabName = "Financials",
                fluidRow(
                    box(title = "Financials", status = "danger", solidHeader = TRUE,
                        collapsible = TRUE,dataTableOutput('financials')),
                    box(title = " Calculate Discounted Cash Flow (DCF)", status = "danger", solidHeader = TRUE,
                        collapsible = TRUE, sliderInput("wacc", "WACC:",
                                                        min = 0, max = 0.5,
                                                        value = 0.1, step = 0.01),sliderInput("g", "Perpetuity Growth:",
                                                                                              min = 0, max = 0.05,
                                                                                              value = 0.02, step = 0.001),
                        
                        sliderInput("growth", "Growth Short Term:",
                                    min = 0, max = 1,
                                    value = 0.1, step = 0.01), numericInput("cashflow", "Cash Flow:", 10, min = 1, max = 1000000),
        ...
        tabItem(tabName = 'Portfolio', 
                fluidRow( 
                    ...
                    box(plotOutput('portfolioreturn'),width = 8),
                    
                    box(title = 'Optima Weights', solidHeader = TRUE, status = 'info',width = 4, verbatimTextOutput('pesos'),verbatimTextOutput('mean')),
                    
                    box(plotOutput('correlacion'),width = 8)
 ...
 output$distPlot <- renderPlot({
                 ...
                 if (input$tipo_grafica == 'Line'){        
                     ...
                     ggplot(prices, aes(x = date, y = close)) +
                         geom_line(color = '#0a8212' ) +  ggtitle(input$download) +
                         xlab("") 
                     ...
                     ggplot(candle, aes(x = date, y = close)) +
                         ...
                         xlab("") +theme_tq() +  ggtitle(input$download)+ theme(panel.background = element_rect(fill = 'gray89'))
...
output$financials <- renderDataTable(cash_flow,  options = list(
                 pageLength = 10))
             
output$valor_empresa <- renderText(valoracion(input$cashflow,input$periodos,input$wacc,input$growth,input$g))

      
 


output$correlacion <-renderPlot(ggcorr(Correlacion(),label = TRUE) + ggtitle('Correlation Matrix')+theme(plot.title = element_text(size = 20)))
             



output$portfolioreturn = renderPlot(ggplot(graficareturns(), aes(y= cumsum(Ra),x=date))+geom_line(color ='#5e96cc', size =1)+
                                        ggtitle('Portfolio Return')+ xlab('')+ylab('')+theme(plot.title = element_text(size = 20)))
